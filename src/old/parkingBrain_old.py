...
def trans_model(s):
    delta_pos = FORWARD_VELOCITY * TIMESTEP_LENGTH
...
def get_desired_table_state(ideal, table):
    ...
    for i in range(len(ideal)):
        if ideal[i] > avg and i > 0 and ideal[i-1] < avg:
            in_room = i
        if ideal[i] < avg and ideal[i-1] > avg:
            out_room = i
            count_rooms += 1
        if count_rooms == table:
            return (in_room + out_room + 1)/2
    return (in_room + len(ideal) + 1)/2
...
def on_step(step_duration):
    ...
    width = (x_max - x_min)/(num_states-1)
    if robot.confident:
        parking_states = robot.parking_spot
        parking_location = parking_states * width + x_min
        if abs(parking_location - robot.pose.x) > 0.40/2 and robot.back_ward:
            #robot.fv=0.2*(parking_location-robot.pose.x)
            robot.fv = 0.2 * (-1 if parking_location < robot.pose.x else 1)
        elif abs(parking_location - robot.pose.x) < 0.40/2 and robot.back_ward:
            robot.fv = 0
